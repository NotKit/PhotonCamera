...
/**
 * Builds JSON text. The port only ever writes, so this is the class that has to
 * match ART's output byte for byte; the reading side is JSONTokener.
 */
public class JSONStringer {
	...
	private JSONStringer close(Scope empty, Scope nonempty, String closeBracket)
	...
	/** JSONArray.join(): a scope that writes values with no brackets. */
	void openBare() {
		stack.add(Scope.NULL);
	}
	...
	/** JSONObject.quote(): the escaper alone, outside any scope. */
	String quoted(String data) {
		int mark = out.length();
		string(data);
		String result = out.substring(mark);
		out.setLength(mark);
		return result;
	}
