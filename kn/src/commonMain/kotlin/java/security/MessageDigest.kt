...
/** SHA-256 only: the one digest the app asks for (the tiled-render harness's
 *  frame hashes).  FIPS 180-4, straight from the spec. */
class MessageDigest private constructor(private val algorithm: String) {
    ...
    fun update(input: ByteArray) = update(input, 0, input.size)
    ...
    fun digest(): ByteArray {
        ...
        while (blockLen != 56) update(byteArrayOf(0))
        val len = ByteArray(8) { (bits ushr (56 - 8 * it)).toByte() }
        update(len)
    ...
    companion object {
        fun getInstance(algorithm: String): MessageDigest {
            if (algorithm.uppercase() != "SHA-256") throw NoSuchAlgorithmException("$algorithm not available")
